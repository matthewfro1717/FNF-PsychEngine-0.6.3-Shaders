...
idleXml = 'BF idle dance0';--you can leave this blank if you're using danceLeft and danceRight
leftXml = 'BF NOTE RIGHT0';
downXml = 'BF NOTE DOWN0';
upXml = 'BF NOTE UP0';
rightXml = 'BF NOTE LEFT0';
...
function onCreate()
	...
	setProperty(tagForChar..'.flipX', true);
	createAnimCases()
	extraAnimCases()
...
function onUpdate(elapsed)
	if curStep >= 1151 then
		...
		doTweenAlpha('unbind1', tagForChar, 1, 40)
        doTweenAngle('floatu1a', tagForChar, 0+45*math.cos((currentBeat+1)*math.pi), 4)
	end
	if curStep == 1151 then
		doTweenAlpha('unbindA', 'dad', 0, 40)
		doTweenAlpha('unbindB', 'iconP2', 0, 40)
	end
end
